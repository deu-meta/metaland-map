...
export default defineComponent({
	...
	setup() {
		const store = useStore(),
			...
			coordinatesControlEnabled = computed(() => !store.state.ui.disableCoordinatesUI && store.getters.coordinatesControlEnabled),
			clockControlEnabled = computed(() => !store.state.ui.disableClockUI && store.getters.clockControlEnabled),
			linkControlEnabled = computed(() => !store.state.ui.disableLink && store.state.components.linkControl),
			chatBoxEnabled = computed(() => !store.state.ui.disableChatUI && store.state.components.chatBox),
